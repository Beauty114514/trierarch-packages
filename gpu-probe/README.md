...
- `host/gpu_probe_host.c` is an Android native library API.  It accepts the
  exported FD, imports it as an EGLImage, and draws that image to a supplied
  `ANativeWindow`.
...
Android host (development machine):

```sh
ANDROID_NDK_HOME=/path/to/ndk bash host/build-android.sh
```

The Android library deliberately exposes a C API only.  A later, explicit
test harness can call it from the existing Wayland host JNI surface without
making the probe part of normal sessions.
